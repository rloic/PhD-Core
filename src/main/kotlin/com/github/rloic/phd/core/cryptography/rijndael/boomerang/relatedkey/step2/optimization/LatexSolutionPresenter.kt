...
class LatexSolutionPresenter(val out: Appendable) : Presenter<Solution> {
    ...
    override fun present(data: Solution) {
        ...
        out.appendBlock((data.config.Nb + 1) * 0, 1, data.sBsubKeyUpper(0), "$ SRK_{0}^{\\Uparrow} $", data.subKeyTable(0))

        out.appendBlock((data.config.Nb + 1) * 3, - 4, data.subKeyLower(0), "$ RK_{0}^{\\Downarrow} $",data.subKeyTable(0))
        out.appendBlock((data.config.Nb + 1) * 3, 1, data.sBsubKeyLower(0), "$ SRK_{0}^{\\Downarrow} $", data.subKeyTable(0))
        ...
        for (i in 0 until data.config.Nr) {
            ...
            if (i < data.config.Nr - 1) {
                ...
                out.appendBlock((data.config.Nb + 1) * 0, 21 * (i + 1) + 1, data.sBsubKeyUpper(i + 1), "$ SRK_{${i + 1}}^{\\Uparrow} $", data.subKeyTable(i + 1))

                out.appendBlock((data.config.Nb + 1) * 3, 21 * (i + 1) - 4, data.subKeyLower(i + 1), "$ RK_{${i + 1}}^{\\Downarrow} $",data.subKeyTable(i + 1))
                out.appendBlock((data.config.Nb + 1) * 3, 21 * (i + 1) + 1, data.sBsubKeyLower(i + 1), "$ SRK_{${i + 1}}^{\\Downarrow} $", data.subKeyTable(i + 1))
            } else {
                out.appendBlock((data.config.Nb + 1) * 0, 21 * (i + 1) - 9, data.subKeyUpper(i + 1), "$ RK_{${i + 1}}^{\\Uparrow} $", data.subKeyTable(i + 1))
                out.appendBlock((data.config.Nb + 1) * 0, 21 * (i + 1) - 4, data.sBsubKeyUpper(i + 1), "$ SRK_{${i + 1}}^{\\Uparrow} $", data.subKeyTable(i + 1))

                out.appendBlock((data.config.Nb + 1) * 3, 21 * (i + 1) - 9, data.subKeyLower(i + 1), "$ RK_{${i + 1}}^{\\Downarrow} $",data.subKeyTable(i + 1))
                out.appendBlock((data.config.Nb + 1) * 3, 21 * (i + 1) - 4, data.sBsubKeyLower(i + 1), "$ SRK_{${i + 1}}^{\\Downarrow} $", data.subKeyTable(i + 1))
            }
