...
class LatexSolutionPresenter(private val out: Appendable) : Presenter<Solution> {
    ...
    data class Column(val header: String, val value: IntMatrix, val table: Matrix<BoomerangTable>? = null)

    private fun setTTFont() = out.append("\\tt\n")

    private fun colorize(table: BoomerangTable?, value: String): String {
        if (value == "0x00") {
            return "\\color{gray}{$value}"
        }

        val color = when (table) {
            BoomerangTable.None, null -> "white"
            BoomerangTable.DDT -> "cat1"
            BoomerangTable.DDT2 -> "cat2"
            BoomerangTable.BCT -> "cat3"
            BoomerangTable.UBCT -> "cat4"
            BoomerangTable.LBCT -> "cat5"
            BoomerangTable.EBCT -> "cat6"
        }

        return "\\cellcolor{$color}{$value}"
    }

    private fun colorize(table: BoomerangTable?, i: Int): String {
    ...
    private fun legend() {
        ...
        out.append(colorize(BoomerangTable.DDT, "0x??") + "&: DDT Transition")
        out.append(" & ")
        out.append(colorize(BoomerangTable.DDT2, "0x??") + "&: DDT2 Transition")
        out.append("\\\\\n")

        out.append(colorize(BoomerangTable.BCT, "0x??") + "&: BCT Transition")
        out.append(" & ")
        out.append(colorize(BoomerangTable.UBCT, "0x??") + "&: UBCT Transition")
        out.append("\\\\\n")

        out.append(colorize(BoomerangTable.LBCT, "0x??") + "&: LBCT Transition")
        out.append(" & ")
        out.append(colorize(BoomerangTable.EBCT, "0x??") + "&: EBCT Transition")
        out.append("\\\\\n")
